        ...
        {!submiting ? (
          <div className="flex flex-col gap-3 w-full">
            ...
            <div className="flex flex-row w-full h-full">
              ...
              {step === 1 ? (
                <>
                  <TitleGeneralized alignText={"left"} > Informació Personal</TitleGeneralized>
                  <form className="flex flex-col gap-3">
                    <label>
                      Nom: 
                      <input className={`${errors.name ? 'bg-pink-100' : 'bg-white'} min-h-10 px-2 text-base mt-4`} placeholder="Nom" 
                      {...register("firstName", {required: "El nom no pot estar buit"})} 
                      />
                    ...
                    <label>
                      Cognoms: 
                      <input className={`${errors.name ? 'bg-pink-100' : 'bg-white'} min-h-10 px-2 text-base mt-4`} placeholder="Cognoms" 
                      {...register("lastName", {required: "Els cognoms no pot estar buit"})} 
                      />
                    ...
                    <label>
                      ...
                      <input
                        ...
                        {...register("password", {
                          ...
                          minLength: { value: 8, message: "Ha de tenir almenys 8 caràcters" },
                          validate: {
                            hasUpperCase: (value) =>
                              /[A-Z]/.test(value) || "Ha de tenir almenys una majúscula",
                            hasLowerCase: (value) =>
                              /[a-z]/.test(value) || "Ha de tenir almenys una minúscula",
                          },
                        })}
                      />
                        ...
                        {...register("birthdate", {
                          ...
                          validate: {
                            isOldEnough: (value) => {
                              ...
                              return age > 14 || (age === 14 && today.getMonth() >= birthDate.getMonth()) || "Has de ser major de 14 anys";
                            },
                          },
                        })}
                      />
                      {errors.birthdate && <span className="text-red-400">{errors.birthdate.message}</span>}
                    </label>

                    <Button disabled={!isValid} className={`bg-primaryHackeps text-white min-h-10 ${!isValid ? 'opacity-50' : ''}`} onClick={() => setStep(2)}>Següent</Button>
                  </form>
                </>) : null}
              {step === 2 ? (
                <>
                <TitleGeneralized alignText={"left"} >Contacte </TitleGeneralized>
                <form className="flex flex-col gap-3">
                <label>
                  Telèfon:
                  <input
              ...
              {step === 3 ? (
                <div className="flex flex-col items-center">
                  ...
                  <TitleGeneralized alignText={"left"} >Avatar</TitleGeneralized>
                  <form className="flex flex-col gap-3">
                    <label>
                      Nickname:
                      <input
                        ...
                        {...register("nickname", { required: "El nickname és obligatori" })}
                      />
                      {errors.nickname && <span className="text-red-400">{errors.nickname.message}</span>}
                    </label>
