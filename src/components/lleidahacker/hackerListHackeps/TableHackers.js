...
const TableHackers = ({mapList}) => {
    ...
    function createGif(text) {
        ...
        } else if (text !== 'Changing..' && text !== "" && text !== 'Acepted' && text !== 'Pending' && text !== 'Rejected') {
        ...
        <table className="w-full ">
            <thead style={{ position: 'sticky', top: 0, backgroundColor: 'white'}}>
                <tr className='text-xl'>
                    ...
                    <th><p>Edat</p></th>
                    <th><p>Talla</p></th>
                    <th><p className='text-center'>Acció</p></th>
                </tr>
            </thead>
            <tbody >
                {participants.map((participant, index) => (
                    ...
                    <tr key={index}>
                        <td>
                            ...
                            <p className={`${createStatusClass(participant.status)} flex flex-row gap-1 items-center mb-0`}>{createGif(participant.status)}{participant.status}</p>
                        </td>
                        ...
                        <td>
                            <div className='flex justify-center'>
                                <div className='bg-primaryLanding text-white rounded-md px-2 py-1 font-semibold cursor-pointer' onClick={() => buttonAction(true, index)}>Acceptar</div>
                                <div className='bg-red-500 text-white rounded-md px-2 py-1 ml-2 font-semibold cursor-pointer' onClick={() => buttonAction(false, index)}>Rebutjar</div>
                            </div>
                        </td>
                        ...
                        <div className='mb-2 flex flex-row gap-1 text-red-500 text-sm items-center ml-1'>
                            {createGif(participant.error)}<p className='my-0.5'>{participant.error} </p> 
                            {participant.error !== "" && (
                                <div onClick={() => {closeError(index)}} className='text-base cursor-pointer hover:text-red-500'>&times;</div>
                            )}  
                        </div>
                    </>
                ))}
