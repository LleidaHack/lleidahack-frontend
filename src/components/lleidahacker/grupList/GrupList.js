...
const GrupList = ({id, me}) => {
    ...
    function getGroupsInfo(){
        ...
        const listGrupsList = [
            {role:"President/a", name:"Junta", image:icon, bgcolor:"grayNeutral", textColor:"black", opacity:100, selected:false},
            {role:"membre", name:"Devs", image:icon, bgcolor:"grayNeutral", textColor:"black", opacity:100, selected:false}, 
            {role:"membre", name:"Marketing", image:icon, bgcolor:"grayNeutral", textColor:"black", opacity:100, selected:false},
            {role:"membre", name:"Contactes", image:icon, bgcolor:"grayNeutral", textColor:"black", opacity:100, selected:false},
            {role:"membre", name:"TechMeetings", image:icon, bgcolor:"grayNeutral", textColor:"black", opacity:100, selected:false},
    ...
    function modifyDatas(key) {
        ...
        if (selected === false) {
            ...
            const newEditionGrups = editionGrups.concat({role:grupsInfo.role, name:grupsInfo.name, status:"Añadidos"});
            setEditionGrups(newEditionGrups);
        } else {
            listGrups[key].selected = false
            const newEditionGrups = editionGrups.filter((grup) => grup.name !== grupsInfo.name);
            setEditionGrups(newEditionGrups)
        }        
    ...
    function changeRol(key, rol){
        ...
        if(rol !== "No asignar"){
            newEditionGrups[key] = { ...newEditionGrups[key], role: rol, status: "Modificado"};
        }else{
            newEditionGrups[key] = { ...newEditionGrups[key], role: rol, status: "Eliminado"};
        }
        setEditionGrups(newEditionGrups);
    }
                        ...
                        <div className='flex flex-col w-full'>
                                                            ...
                                                            <div className='cursor-pointer'>
                                                                ...
                                                                (<>
                                                                    <i className="fa-solid fa-sort-up" onClick={() => setVisibilityOptions((prevVisibilityOptions) => {
                                                                        ...
                                                                        newVisibilityOptions[key] = "hidden";
                                                                ...
                                                                (<>
                                                                    <i className="fa-solid fa-sort-down" onClick={() => setVisibilityOptions((prevVisibilityOptions) => {
                                                                        const newVisibilityOptions = [...prevVisibilityOptions];
                                                                        newVisibilityOptions[key] = "block";
                                                                        return newVisibilityOptions;
                                                                    })}></i>
                                                                </>)}
                                                            </div>
                                                            <div className={`h-fit w-fit border-1 pt-1 pr-4 rounded-md text-gray-500 text-justify relative ${visibilityOptions[key] ? visibilityOptions[key] : "hidden"}`}  >
                                                                {grup.name === "Junta" ? 
                                                                (<>
                                                                    <ul>
                                                                        <li className='hover:text-primaryLanding' onClick={() => { changeRol(key, "President");}}>President/a</li>
                                                                        <li className='hover:text-primaryLanding' onClick={() => {changeRol(key, "Tresorer");}}>Tresorer/a</li>
                                                                        <li className='hover:text-primaryLanding' onClick={() => {changeRol(key, "Secretari");}}>Secretari/a</li>
                                                                        <li className='hover:text-primaryLanding' onClick={() => {changeRol(key, "No asignar");}}>No asignar</li>
                                                                    </ul>
                                                                </>) :
                                                                (<>
                                                                    <ul>
                                                                        <li className='hover:text-primaryLanding' onClick={() => { changeRol(key, "Cap d'equip");}}>Cap d'equip</li>
                                                                        <li className='hover:text-primaryLanding' onClick={() => {changeRol(key, "Membre");}}>Membre</li>
                                                                        <li className='hover:text-primaryLanding' onClick={() => {changeRol(key, "No asignar");}}>No asignar</li>
                                                                    </ul>
                                                                </>)}
                                                            </div>
                            ...
                            {editionGrups.length > 0 &&
                                <div className='flex flex-row w-full justify-end'>
                                    <button className='bg-primaryLanding text-white rounded-lg px-4 py-2 mr-6' onClick={saveChanges}>Guardar</button>
                                </div>
                            }
                        </div>
                    </div>
