        ...
        <div className="flex flex-col h-screen">
                ...
                <div className='content px-16 py-3'>
                    <div className='descripcio'>
                        <h2 className='text-3xl font-bold mb-4'>Descripció de l'esdeveniment</h2>
                        <p>{description}</p>
                    </div>
                    <div className='detalls-esdeveniment  grid grid-cols-* grid-rows-4 gap-x-auto gap-y-2 mt-12'>
                        <h2 className='text-3xl font-bold mb-4 col-span-3'>Detalls de l'esdeveniment</h2>
                        <p><strong>Data de creació:</strong> {event?.creationDate}</p>
                        <p><strong>Ubicació:</strong> {event?.location}</p>    
                        <p><strong>Durada:</strong> {event?.duration}</p>
                        <p><strong>Max Participants:</strong> {event?.maxParticipants}</p>
                        <p><strong>Participants:</strong> {event?.participants}</p>
                        <p><strong>Max sponsors</strong>{event?.maxSponsors}</p>
                        <p><strong>Sponsors:</strong> {event?.sponsors}</p>
                        <p><strong>Data Inici inscripcions:</strong> {event?.joindDate}</p>
                        <p><strong>Data Fi inscripcions:</strong> {event?.endJoinDate}</p>
                    </div>
                    <div className='participants mt-8'>
                        <h2 className='text-3xl font-bold mb-4'>Participants</h2>
                        <ButtonLleidahack primary white onClick={() =>navigate(`/events/${eventId}/participants`)}>
                    ...
                    <ButtonLleidahack roundedLanding onClick={() => openPopup()}>
                        <span role="img" aria-label="gear">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0a1.724 1.724 0 002.573.982c.797-.46 1.757.388 1.518 1.25a1.724 1.724 0 002.062 2.062c.862-.239 1.71.721 1.25 1.518a1.724 1.724 0 00.982 2.573c.921.3.921 1.603 0 1.902a1.724 1.724 0 00-.982 2.573c.46.797-.388 1.757-1.25 1.518a1.724 1.724 0 00-2.062 2.062c.239.862-.721 1.71-1.518 1.25a1.724 1.724 0 00-2.573.982c-.3.921-1.603.921-1.902 0a1.724 1.724 0 00-2.573-.982c-.797.46-1.757-.388-1.518-1.25a1.724 1.724 0 00-2.062-2.062c-.862.239-1.71-.721-1.25-1.518a1.724 1.724 0 00-.982-2.573c-.921-.3-.921-1.603 0-1.902a1.724 1.724 0 00.982-2.573c-.46-.797.388-1.757 1.25-1.518a1.724 1.724 0 002.062-2.062c-.239-.862.721-1.71 1.518-1.25.797.46 1.757-.388 2.573-.982z" />
