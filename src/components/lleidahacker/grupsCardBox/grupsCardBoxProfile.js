...
const GrupsCardBoxProfile = ({name, role, bgcolor, opacity, image, size, textColor, changeColor, click}) => {
    ...
    return (
        <div className={`select-none flex flex-row items-center align-center bg-${bagColor} rounded-md w-${sizebox === "small" ? "64" : "72"} h-${sizebox === "small" ? "24" : "36"}`} style={{ opacity: opacity }} onClick={clicked}>
            ...
            <div className={`flex flex-col items-left ml-3 text-${textColor !== undefined ? textColor : "white"}`}>
