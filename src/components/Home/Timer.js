  ...
  if (timestampDay >= nowDay && active) {
    return (
      <div className="countdown-timer">
        ...
        {remainingTime.days ? <span>{padWithZeros(remainingTime.days)}</span>:<></>}
        {remainingTime.days ? (
          <span style={{ fontSize: "2vw" }}>
            di{remainingTime.days === 1 ? "a" : "es"}
          </span>
        ) : <></>}
        {remainingTime.hours ? <span>{padWithZeros(remainingTime.hours)}</span> : <></>}
        {remainingTime.hours ? (
          <span style={{ fontSize: "2vw" }}>
    ...
    return (
      <div className="countdown-timer">
        ...
        {remainingTime.minutes ? <span>{padWithZeros(remainingTime.minutes)}</span> : <></>}
        {remainingTime.minutes ? (
          <span style={{ fontSize: "2vw" }}>
            minut{remainingTime.minutes === 1 ? "" : "s"}
          </span>
        ) : <></>}
        {remainingTime.seconds ? <span>{padWithZeros(remainingTime.seconds)}</span> : <></>}
