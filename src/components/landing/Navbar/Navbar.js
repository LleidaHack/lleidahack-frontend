...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { label: "Contacte", href: "/lleidahack/contacte", external: false },
    { label: "Qui som?", href: "/lleidahack/qui-som", external: false },
  ];

  const isActive = (href) => location.pathname === href.replace("/lleidahack", "");
  ...
  return (
    <nav
      ...
      style={!scrolled ? { background: "rgba(35,35,35,0.6)", backdropFilter: "blur(8px)" } : {}}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">

          {/* Logo */}
          <a href="/lleidahack" className="flex items-center gap-3 flex-shrink-0">
            <img src={logoLleidaHack} alt="LleidaHack logo" className="h-9 w-9" />
            <img
              ...
              link.external ? (
                ...
                <a
                  ...
                  className={`text-sm font-medium transition-colors duration-200 no-underline relative group ${
                    isActive(link.href) ? "text-white font-bold" : "text-white opacity-80 hover:opacity-100"
                  }`}
                >
                  {link.label}
                  <span
      ...
      <div
        ...
        style={{ background: "rgba(35,35,35,0.98)", backdropFilter: "blur(16px)" }}
      >
