...
export const searchItems = async () => {
    const {results, setResults, categoryList, dateList, typeList, searchTerm, contextType, setResult, hasCategory, hasDates, hasType} = useSearch();
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            if (contextType === "Noticies") {
                ...
                const hackerListGrouped = await get_hackers_participants_gruped_list(eventId);
                setAllResults({
    ...
    useEffect(() => {
        ...
        async function filterData(){
            ...
            if (hasTrueValue){
                let categories = categoryList.filter(category => category.value).map(category => category.key);
